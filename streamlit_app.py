...
# Gemini SDK is optional - the app still runs without it, the Food Scanner
# page just shows an install instruction until it's installed.
try:
...
if 'last_food_analysis' not in st.session_state:
...
FOOD_ANALYSIS_PROMPT = """You are a nutrition assistant helping a diabetes patient understand a food photo.
Identify the food item(s) in the image, then respond with ONLY valid JSON (no markdown fences, no extra text) matching exactly this schema:

{
  "food_name": "string - the main food identified",
  "other_items": ["array of other visible food items, empty list if none"],
  "estimated_carbs_g": number,
  "estimated_sugar_g": number,
  "estimated_calories": number,
  "glycemic_impact": "Low or Medium or High",
  "diabetes_friendly": "Yes or Moderate or No",
  "portion_assumed": "short string describing the portion size you assumed",
  "recommendation": "one or two sentence practical tip for someone managing diabetes"
}

Base estimates on a typical single visible serving. If the image does not clearly show food, set food_name to "Unrecognized" and explain why in recommendation."""


def analyze_food_image(image_bytes, api_key, model_name="gemini-2.5-flash"):
    """Send a food photo to the Gemini vision API and return the raw text response."""
...
def show_food_scanner():
    ...
    <div class="info-box">
    <h4>🔍 How it works</h4>
    <p>Upload or snap a photo of your meal. A computer-vision model identifies the food and 
    estimates its carb/sugar impact, so you know how it may affect your blood sugar — before you eat it.</p>
    </div>
    """, unsafe_allow_html=True)

    with st.expander("🔑 One-time API setup"):
        st.markdown("""
        This feature uses **Google's Gemini vision API** for food recognition.
        Get a free API key from [Google AI Studio](https://aistudio.google.com/apikey), then paste it below.
        Your key is kept only in this browser session — it is never saved to disk.
        """)
        ...
        st.session_state.gemini_model = st.selectbox(
            ...
            index=["gemini-2.5-flash", "gemini-3.5-flash"].index(st.session_state.gemini_model)
        )

    if not GENAI_AVAILABLE:
        st.markdown("""
        <div class="warning-box">
        <h3>⚠️ Missing dependency</h3>
        <p>Install the Gemini SDK to enable this feature, then restart the app:</p>
        </div>
        """, unsafe_allow_html=True)
        st.code("pip install google-genai pillow", language="bash")
        return
    ...
    image_file = uploaded or camera_img
            ...
            if not st.session_state.gemini_api_key:
                ...
                with st.spinner("Analyzing your food..."):
                    ...
                    try:
                        ...
                        raw_text = analyze_food_image(
                            image_bytes,
                            st.session_state.gemini_api_key,
                            st.session_state.gemini_model,
                        )
                        cleaned = re.sub(r"^```json|```$|^```", "", raw_text.strip(), flags=re.MULTILINE).strip()
                        st.session_state.last_food_analysis = json.loads(cleaned)
                    except json.JSONDecodeError:
                        st.error("Couldn't parse the AI's response. Try again or use a clearer photo.")
                        if raw_text:
    ...
    if st.session_state.last_food_analysis:
        data = st.session_state.last_food_analysis
        impact = data.get("glycemic_impact", "Medium")
        impact_class = {"Low": "risk-card-low", "Medium": "risk-card-medium", "High": "risk-card-high"}.get(impact, "risk-card-medium")

        st.markdown(f"""
        <div class="{impact_class}">
        <h2>🍽️ {data.get('food_name', 'Unknown')}</h2>
        <p>Glycemic Impact: {impact}</p>
        </div>
        """, unsafe_allow_html=True)

        c1, c2, c3 = st.columns(3)
        c1.metric("Carbs", f"{data.get('estimated_carbs_g', '?')} g")
        c2.metric("Sugar", f"{data.get('estimated_sugar_g', '?')} g")
        c3.metric("Calories", f"{data.get('estimated_calories', '?')} kcal")

        other = data.get("other_items", [])
        if other:
            st.markdown(f"**Other items detected:** {', '.join(other)}")

        st.markdown(f"""
        <div class="feature-card">
        <h3>💡 Recommendation</h3>
        <p>{data.get('recommendation', '')}</p>
        <p style="color:#6b7280;font-size:0.95rem;">Portion assumed: {data.get('portion_assumed', 'a single standard serving')}</p>
        </div>
        """, unsafe_allow_html=True)

        friendly = data.get("diabetes_friendly", "Moderate")
        if friendly == "Yes":
            st.success("✅ This looks diabetes-friendly in moderation!")
        elif friendly == "No":
            st.warning("⚠️ This food may spike your blood sugar significantly — eat sparingly and monitor glucose.")
        else:
            st.info("ℹ️ Moderate impact — okay in controlled portions, pair with fiber/protein to blunt the spike.")

        if st.button("✅ Log this meal (+10 points)"):
            st.session_state.total_points += 10
            st.success("+10 points! Meal logged.")

    st.markdown("""
    <div class="warning-box">
    <p><strong>⚠️ Disclaimer:</strong> AI estimates are approximate and for educational purposes only. 
    Always verify carb counts using product labels, and consult a registered dietitian for insulin dosing decisions.</p>
    </div>
    """, unsafe_allow_html=True)
...
def show_home(df):
    ...
    with col1:
        ...
        <ul style="font-size:1.1rem;line-height:2;">
            ...
            <li><strong>📷 AI Food Scanner:</strong> Snap a meal photo for instant carb/sugar analysis</li>
